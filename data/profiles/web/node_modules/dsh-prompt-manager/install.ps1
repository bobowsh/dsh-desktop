...
$LocalPluginDir = [System.IO.Path]::GetFullPath((Join-Path $DshHome 'local-plugins\dsh-prompt-manager'))
...
$alreadyInstalled = $dependencies -and $dependencies.PSObject.Properties['dsh-prompt-manager']
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw 'pnpm add dsh-prompt-manager failed.' }
} finally {
  Pop-Location
}
Write-Host "[1/2] Refreshed local dependency: file:$depPath"

if ($patch -notmatch '(?m)^\s*name:\s*dsh-prompt-manager\s*$') {
  $row = @('- insert:', '    - id: prompt-manager', '      name: dsh-prompt-manager') -join [Environment]::NewLine
  ...
  Write-Host '[2/2] Registered prompt-manager.'
} else {
  if ($patch -cne $originalPatch) { Write-Utf8NoBom $PatchPath $patch }
  Write-Host '[2/2] prompt-manager is already registered.'
}
