...
export function readOpencodeDb(dbPath, options = {}) {
  ...
  try {
    ...
    for (const row of sessionRows) {
      ...
      // options.filter（如 mimocode 的后台任务会话过滤）：剔除谓词——返回 true 的
      // 会话不进入结果集（导入/预览/发现共用 readOpencodeDb，一处过滤全覆盖）。
...
//   checkpoint-writer —— 标题前缀 "checkpoint-writer: ..."（351 条），消息 agent=checkpoint-writer
...
// mimocode 是 opencode 的 fork，目录里库文件名不同（mimocode.db）——经 options.dbName
// 参数化（默认 opencode.db），读取/导入编排完全复用；options.filter 透传给
// readOpencodeDb（mimocode 传 isMimocodeBackgroundSession 剔除后台任务会话）。
