...
// 各源格式的 `convertXxx(raw, args)`（见同目录 convert/*.mjs，现共 13 种：
// claude/codex/chatgpt/cursor/gemini/reasonix/opencode/zcode/grokbuild/openclaw/
// hermes/pi/kimi）把原始 transcript 文本解析成统一的回合中间结构，再交给共享的
// synthesizeSession 合成 DSH 事件日志，保证所有源事件纪律一致。
...
// ── 会话事件结构校验（REQ-57）────────────────────────────────────────────
// 轻量自检：seq 连续无重复、事件类型白名单、surface 事件带 surfaceOp:'append'、
// tool/result 的 sourceEventSeqs 必须指向集合内存在的 tool/call。指向集合外的
