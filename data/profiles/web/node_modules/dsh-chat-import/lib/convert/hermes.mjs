...
export function convertHermesJson(raw, args = {}) {
  ...
  if (chat && typeof chat === 'object' && !Array.isArray(chat) && Array.isArray(chat.messages)) {
    ...
    for (const m of chat.messages) {
      ...
      // REQ-50：readHermesDb 变体列（toolCalls / reasoning）透传给消息形状
      messages.push({
        role: m.role,
        content: m.content,
        ts: m.ts,
  ...
  for (const m of messages) {
    ...
    if (m.role === 'user') {
      ...
      // REQ-50 hermes-agent 变体：reasoning 独立列 → 前置 reasoning 块
      ...
      // REQ-50：tool_calls 独立列（JSON 文本已在读取层解析）→ tool-call 块
