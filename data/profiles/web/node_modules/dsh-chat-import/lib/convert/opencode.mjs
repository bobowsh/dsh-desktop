...
export function convertOpencodeJson(raw, args = {}) {
  ...
  const out = synthesizeSession({
    ...
    turns: seedTurns,
    title,
    // mimocode 是 opencode 的 fork（SQLite schema 同构），复用同一转换器，
    // provider 标签由调用方经 args.provider 指定（默认 opencode）。
...
/** mimocode（opencode fork）会话 → DSH 会话：复用 convertOpencodeJson，仅换 provider 标签。 */
