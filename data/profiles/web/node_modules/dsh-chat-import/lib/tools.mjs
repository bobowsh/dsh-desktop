...
import { importOpencodeFile, importOpencodeDirectory, isMimocodeBackgroundSession } from './opencode.mjs'
...
export function registerTools(ctx, registryDir) {
  ...
  // 无 model 列、库文件名不同），读取/导入/预览完全复用 opencode 管线——只换默认库
  // 文件名（mimocode.db，目录模式定位）与 provider 标签（mimocode）。默认位置
  ...
  ctx.tools.register(makeImportTool(ctx, {
    ...
    io: {
      file: (c, t, a) => importOpencodeFile(c, t, a, { registryDir, runDecision, markTrimmedSource, filter: isMimocodeBackgroundSession }),
      dir: (c, d, a) => importOpencodeDirectory(c, d, a, { registryDir, runDecision, markTrimmedSource, dbName: 'mimocode.db', filter: isMimocodeBackgroundSession }),
