...
import { readOpencodeDb, isMimocodeBackgroundSession } from './opencode.mjs'
...
// mimocode 预览：opencode fork，同构 SQLite 只读重演（readOpencodeDb 已兼容无 model
// 列的 mimocode schema），仅库文件名（mimocode.db）与 provider 标签（mimocode）不同。
export async function previewMimocodeFile(ctx, target, args) {
  const path = target.displayPath || ctx.fs.processPath(target)
  const sessions = readOpencodeDb(path, { fullHistory: args.fullHistory === true, filter: isMimocodeBackgroundSession })
  const wanted = Array.isArray(args.sessionIds) && args.sessionIds.length > 0 ? new Set(args.sessionIds) : null
  const results = []
  for (const s of sessions) {
    if (wanted && !wanted.has(s.id)) continue
