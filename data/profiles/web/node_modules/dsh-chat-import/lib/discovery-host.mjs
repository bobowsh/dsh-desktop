...
function dbSessionSummaries(kind, dbPath) {
  try {
    ...
    // mimocode 是 opencode fork（schema 同构），共用 readOpencodeDb（已兼容无 model 列）；
    // 过滤 MiMo 后台任务会话（checkpoint-writer / AutoDream / AutoDistill），发现列表不展示
    if (kind === 'mimocode') {
      return readOpencodeDb(dbPath, { filter: isMimocodeBackgroundSession }).map((s) => dbSummary(s, 'createdAt'))
    }
    if (kind === 'zcode') {
