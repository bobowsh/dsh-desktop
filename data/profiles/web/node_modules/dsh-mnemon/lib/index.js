...
Completion protocol: call \`${resultToolName}\` exactly once with the final result matching its parameter schema. This is the only completion channel for this run. Do not finish with a plain-text answer.`;
			const perOpMaxTokens = operation === "migration" ? 16384 : operation === "compaction" || operation === "document-archive" ? 8192 : operation === "metadata-maintenance" ? 4096 : void 0;
...
function textLength(messages) {
	return messages.filter((message) => message.source.kind === "user").map((message) => message.content.filter((block) => block.type === "text" && "text" in block && typeof block.text === "string").map((block) => block.text).join("\n").trim().length).reduce((total, length) => total + length, 0);
}
