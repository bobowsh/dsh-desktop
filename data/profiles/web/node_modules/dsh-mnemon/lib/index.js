...
function runtimeMemoryPromptText(runtimeMemory) {
	return runtimeMemory.contextText().replaceAll(LITERAL_EMPTY_BRACES, `{{${RUNTIME_MEMORY_EMPTY_BRACES_VARIABLE}}}`);
}
...
var MnemonService = class {
	...
	async search(request, signal) {
		...
		for (const [bodyOrder, { body, result }] of batches.entries()) {
			results.push(...result.results.map((entry, index) => ({
				...this.annotate(entry, body),
		...
		results.sort((left, right) => heterogeneous ? (right.federatedScore ?? 0) - (left.federatedScore ?? 0) || left.bodyOrder - right.bodyOrder : (right.score ?? 0) - (left.score ?? 0));
		return {
			query,
			mode,
...
${indentedText(request.content ?? "")}`;
	...
	async delegate(parent, operation, label, prompt, tools, outputSchema, signal, preferredProvider = "spawn", persona = WRITE_PERSONA) {
		...
		try {
			...
			if (result.stopReason !== "completed" || result.structured === void 0) {
...
//#endregion
export { Config, DocumentManager, InteractionConfig, LiveMnemonRuntime, MnemonLifecycle, MnemonPackManager, MnemonService, MnemonSubagentCoordinator, RuntimeMemoryController, StorageScopeInspector, VersionUpdateManager, apply, createRunner, createRuntimeGraph, inject, name, resolveConfig, resolveInteractionConfig };
